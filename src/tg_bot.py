...
class TG_BOT:
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if resp.status_code in [200, 201]:
                    ...
                    if user_info.get("can_create_review", False):
                        await message.answer(
                            text=f"{TG_BOT.WELCOME_TEMPLATE.format(first_name=first_name)}\n\n{TG_BOT.ADMIN_PANEL_MESSAGE}",
                            reply_markup=self._admin_keyboard()
                        )
                    else:
                        await message.answer(
                            TG_BOT.WELCOME_TEMPLATE.format(first_name=first_name)
                        )
    ...
    async def edit_review_callback(self, callback: CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            logger.error(f"Ошибка при получении информации о ревью: {e}")
            await callback.message.edit_text(
                f"✏️ Редактирование ревью {review_id}\n\n"
...
async def main():
    ...
    bot = TG_BOT(bot_token=bot_token)
